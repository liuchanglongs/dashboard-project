...
如果你对 echarts 还不熟悉，官网的[快速上手](https://echarts.apache.org/handbook/zh/get-started)能让你快速的了解 echarts，其实基本所有的 echarts 处理都能简单的分解为三步：

1. 获取 DOM 实例，通过`echarts.init()`方法，生成`echarts`实例
...
echarts 包含的内容很多，我们没有必要完全引入，可以按需引入的接口来打包必须的组件
...
- 地图的实现：echarts、百度等地图 jdk、threejs
- 地图的静态数据https://datav.aliyun.com/portal/school/atlas/area_selector
...
const { data: userDetail, execute: fetchUser } = useAsync(
...
const {
  ...
  refresh: reloadTable,
} = useAsync((params: QueryParams) => getTableList(params), {
